...
#' @param pa probability of being available for a lab test
#' @param pb probability of being available for hospitalization
#' @param theta1 capture probability of lab test
#' @param theta2 capture probability of hospitalization
#'
#' @return list of capture and availability histories
#' @export
...
make_history <- function(N, M, k, pa, pb, theta1, theta2) {
  ...
  h_2 <- matrix(0, nrow = N, ncol = k) # contains 2s for recaptures
  
  #' first, simulate latent a and b and use them to create h and h_2
  for (i in 1:N) {
    for (j in 1:k) {
      if (j==1) {
        # must populate b first: if available for both lab test and hospital in the same time period then only count b
        b[i,j] <- 0
      ...
      # for h_2, indicate capture by 1 and recapture by 2 
      # in h, both are 1s
  ...
  #' a 1 in position #2 in obs_a indicates that a person is definitely around for lab testing during time period #2
  #' see `populate_row_obs_ab()` for examples
